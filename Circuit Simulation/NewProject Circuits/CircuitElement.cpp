...
bool CircuitElement::IsDepSource()
{
	 if((ElementName[0]=='v'||ElementName[0]=='V')&&(ElementName[2]=='V'||ElementName[2]=='v'))
		return true;
	else if((ElementName[0]=='v'||ElementName[0]=='V')&&(ElementName[1]=='C'||ElementName[1]=='c')&&(ElementName[2]=='c'||ElementName[2]=='C'))
		return true;
	else 
		return false;
...
{
	Value=complex<float> (val*cos((Phase/180)*22/7),val*sin((Phase/180)*22/7));
}
